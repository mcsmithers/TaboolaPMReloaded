...
$(document).ready(function () {
    ...
    function performCleanupAndCalcs(merged) {
        ...
        merged = merged.map(element => {
            ...
            const cpc = !isNaN(cost / clicks) || !isFinite(cost / clicks) ? Number(cost / clicks).toFixed(2) : '';

            const rpc = !isNaN(revenue / clicks) || !isFinite(revenue / clicks) ? Number(revenue / clicks).toFixed(2) : '';

            const rpa = !isNaN(revenue / actions) || !isFinite(revenue / actions) ? Number(revenue / actions).toFixed(2) : '';
