...
$(document).ready(function () {
    ...
    function buildTableBodyEntries(merged) {
        ...
        // Assist with fuzzy search
        var fuseOptions = { keys: ["name", "affilate", "offer", "date"] };
        var options = { display: "name", key: "affId_offId", fuseOptions: fuseOptions };
        $(".search").fuzzyComplete(airports, options);
        $('.search').on('keyup blur', function() {
          $(this).parent().find(".tr").html($(this).parent().find("select").val());
        });
