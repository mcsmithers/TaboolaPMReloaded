...
$(document).ready(function () {
    ...
    function buildTableBodyEntries(merged) {
        ...
        // // this makes the table work, but limits the entires in there on re-draws
        //     // clear existing table
        //     d3.select('#table').select('table').remove();
        //     // create new table
            ...
            $table.bind('repaginate', function () {
                if (pages > 1) {
                    ...
                    for (var page = startPager; page < endPager; page++) {
                        $('<button id="pager-button" class="button small"><span id="pg' + page + '" class="' + (page == currentPage ? 'pg-selected' : 'pg-normal') + '"></span></button>').text(page + 1).bind('click', {
                            newPage: page
                        }, function (event) {
