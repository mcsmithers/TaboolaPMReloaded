...
$(document).ready(function () {
    ...
    function buildTableBodyEntries(merged) {
        ...
        d3.select("#buttons").select("#previous-button").on("click", function (d) {
            console.log('next', d.portion);
            if (d.portion - rowsPerPage >= 0) {
                d.portion -= rowsPerPage;
                redraw(d.portion);
            }
        });
        ...
        d3.select("#buttons").select("#next-button").on("click", function (d) {
            ...
            console.log('previous', d.portion, rows, totalRows);
            if (d.portion < (totalRows - rowsPerPage)) {
                d.portion += rowsPerPage;
                redraw(d.portion);
            }
        });
        ...
        function redraw(start) {
            d3
                ...
                .style("display", function (d, i) {
                    return i >= start && i < start + rowsPerPage ? null : "none";
                });
        }
        ...
        // $('#table').prepend('<div class="count">' + allRows + ' total results</div>');
        // console.log('total stuff ', nested.length);

        // // here's paginating with d3 that somewhat works
        // d3.select("#buttons").datum({
        //     portion: 0
        // });
        // // the chain select here pushes the datum onto the up and down buttons also
        ...
        //     //     var pages = Math.ceil($table.find("tbody").length / itemsPerPage);
        //     // }
        //     // visible row count
        //     // var shownRows = $('#table tr.detail-row:visible').length;
        ...
        //             var startPager = currentPage > 2 ? currentPage - 2 : 1;
        //             console.log('start pager ' + startPager);
        //             var endPager = startPager > 1 ? currentPage + 3 : 5;
        //             console.log('end pager ' + startPager);
        //             if (endPager > pages) {
        //                 endPager = pages;
        //                 startPager = pages - 5;
        //                 if (startPager < 1)
        ...
        //                 $('<button id="pager-button" class="button tiny"><span id="pg' + page + '" class="' + (page == currentPage ? 'pg-selected' : 'pg-normal') + '"></span></button>').text(page).bind('click', {
        //                     newPage: page
        //                 }, function (event) {
        ...
        //                 currentPage = pages - 1;
        //                 $table.trigger('repaginate');
        //                 // var rowCount = $('#table tbody tr.detail-row').length;
        //                 // var numOfVisibleRows = $('tbody tr.detail-row:visible').length;
        //             }).appendTo(pager);
        ...
        //         }
        //         $table.find(
        //             'tbody tr.summary:not(:has(th))').hide().slice(currentPage * itemsPerPage, (currentPage) * itemsPerPage).show();
        ...
        //     $table.trigger('repaginate');
        //     // Show a result count to help show
        //     // var shownRows = merged.length;
        //     // $('#table').prepend('<div class="count">' + shownRows + ' total results</div>');

        //     // then just hide as needed
        //     if (startDateSelect != endDateSelect) {
        //         $("tr.detail-row").css("display", "none"); // hides the row
        //     }
        // });
