...
$(document).ready(function () {
    ...
    function failFunction(response) {
        ...
        $('#error-box').show();
        $('.loader').remove();
        $('#errorContainer').prepend('<img id="error" src="shining.gif" />')
        $('#errorContainer').html('Oh no! Something broke. <br/><b>Error: ' + response.statusCode + '</b>.  Try refreshing the page.');
    ...
    function buildTableBodyEntries(merged) {
        ...
        // // Paginate the table, there's a plugin but it's not that easy to use
        $('.paginated').each(function () {
            ...
            // }
            // if (startDateSelect == endDateSelect) {
            //     $("tr.summary").css("display", "none"); // hides the row
            // }
            $table.bind('repaginate', function () {
                if (pages > 1) {
                    ...
                    $('<button class="button hollow"></button>').text(' « First ').bind('click', function () {
                        currentPage = 0;
                    ...
                    $('<button class="button hollow"> « Prev </button>').bind('click', function () {
                        if (currentPage > 0)
                            currentPage--;
                        $table.trigger('repaginate');
                        // if (startDateSelect != endDateSelect) {
                    ...
                    $('<button class="button hollow"> Next » </button>').bind('click', function () {
                    ...
                    $('<button class="button hollow"> Last » </button>').bind('click', function () {
                        currentPage = pages - 1;
                        $table.trigger('repaginate');
                        // if (startDateSelect != endDateSelect) {
                        //     $("tr.detail-row").css("display", "none"); // hides the row
                ...
                $table.find(
                    'tbody tr.detail-row:not(:has(th))').hide().slice(currentPage * itemsPerPage, (currentPage + 1) * itemsPerPage).show();
            });
        ...
        $('#searchbox').keyup(function () {
            ...
            // update the rsults count
            var countedObj = {};
            var numOfVisibleRows = $('tr:visible').length;
            const countedResults = $('div#result-count').text('Showing ' + (numOfVisibleRows - 1) + ' of ' + (numOfVisibleRows - 1) + ' entries');
            if (countedResults != null) {
                countedResults.each(function () {
                    var text = $.trim($(this).text());
                    if (countedObj[text]) {
                        $(this).remove();
                    } else {
                        countedObj[text] = true;
                    }
                })
            }
        });
    ...
    Promise.all([
        ...
        .then(data => {
            ...
            $('.loader').remove();
            $('#errorContainer').prepend('<img id="error" src="shining.gif" />')
            $('#errorContainer').html('Oh snap, something broke.  Try refreshing the page. Error: Yahoo is not available right now');
            $('#errorContainer').css('display', 'block');
            $('#error-box').css('display', 'block');
    ...
    $("#start-date").change(function () {
        ...
        Promise.all([
            ...
            .then(data => {
                ...
                $('#table').remove();
                $('#errorContainer').html('Oh noes, something broke. Try again later or refresh the page. <b>Error: Yahoo is unavailable.  Try again.</b>');
                $('#error-box').css('display', 'block');
                $('#errorContainer').css('display', 'block');
                document.getElementById("errorContainer").style.color = "red";
                console.log(err);
    ...
    $("#end-date").change(function () {
        ...
        Promise.all([
            ...
            .then(data => {
                ...
                $('#table').prepend('<img id="error" src="shining.gif" />')
                $('#table').remove();
                $('#errorContainer').html('Oh no! Something broke. <br/><b>Error:  Yahoo is not responding</b>.  Try refreshing the page.');
                $('#errorContainer').css('display', 'block');
                $('#error-box').css('display', 'block');
                document.getElementById("errorContainer").style.color = "red";
                console.log(err);
