...
$(document).ready(function () {
    ...
    const getTune = (startDateSelect, endDateSelect) => $.ajax({
        ...
        .then(response => {
            ...
            const tune = tuneData.map(function (value, key) {
                ...
                entry.offerId = offerId;
                entry.date = date;
                entry.actions = Number.parseInt(actions);
    ...
    const getTaboola = () => $.ajax({
        ...
        .then(response => {
            ...
            const taboola = taboolaReportData.map(function (value, key) {
                ...
                entry.date = date;
                // entry.affId_offId = affId_offId;
                entry.cost = Number.parseFloat(cost);
                console.log('pre-merge cost ', cost, ' for ', entry.affId_offId);
                entry.campaign = campaign;
                entry.clicks = Number.parseFloat(clicks);
    ...
    function performCleanupAndCalcs(merged) {
        ...
        merged = merged.map(element => {
            ...
            const clicks = !isNaN(element.clicks) ? Number.parseFloat(element.clicks) : '';
        
            const actions = !isNaN(element.actions) ? Number.parseFloat(element.actions) : '';
        
            const cost = !isNaN(element.cost) ? Number.parseFloat(element.cost) : '';
        
            const revenue = !isNaN(element.revenue) ? Number.parseFloat(element.revenue): '';
        
            const name = !isNaN(element.name) ? element.name : '';

            const date = element.date;

            const affId_offId = element.affId_offId;
        
            const affiliate = !isNaN(element.cost) ? element.affiliate : '';
        
            const offer = !isNaN(element.cost) ? element.offer : '';
        
            const profit = !isNaN(revenue - cost) ?  Number.parseFloat(revenue - cost) : '';

            const profitMargin = !isNaN(Number.parseInt(profit / revenue)) ? Number.parseInt(profit / revenue) : '';
                
            const cpc = !isNaN(Number.parseFloat(cost / clicks)) || !isFinite(Number.parseFloat(cost / clicks)) ? Number.parseFloat(cost / clicks).toFixed(2) : '';
        
            const rpc = !isNaN(Number.parseFloat(revenue/ clicks)) || !isFinite(Number.parseFloat(revenue / clicks)) ? Number.parseFloat(revenue / clicks).toFixed(2) : '';

            
            const rpa = (parseFloat(revenue / actions).toString().match(/\.\d{2}$/) || {input: ""}).input;
            // const rpa = !isNaN(Number.parseFloat(cost / actions)) || !isFinite(Number.parseFloat(cost / actions)) ? Number.parseFloat(cost / actions).toFixed(2) : '';
        ...
        merged = merged.filter(m => {
            return (m.affiliate !== '' && m.offer !== '' && m.cost !== 0); // v2 will have the option to do either all 3 filter or just to 2 so we see the entires with 0 cost
