...
$(document).ready(function () {
    ...
    function performCleanupAndCalcs(merged) {
        ...
        merged = merged.map(element => {
            ...
            const cpc = parseFloat(cost / clicks).toFixed(2).match(/\.\d{2}/)  || "";

            const rpc = !isNaN(Number.parseFloat(revenue / clicks)) ? Number.parseFloat(revenue / clicks).toFixed(2) : '';            
            
            const rpa = !isNaN(Number.parseFloat(revenue / actions)) ? Number.parseFloat(revenue / actions).toFixed(2) : '';
