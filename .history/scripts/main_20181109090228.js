...
$(document).ready(function () {
    ...
    //     // // Now that we have what we need, let's filter out the inactive accounts from the affiliates and offers that are blank
    //     merged = merged.filter(({ affiliate, offer, cost }) => !(affiliate === '' || offer === '' || cost === 0 || affiliate === undefined || offer === undefined));
    //     // merged = merged.filter(m => {
    //     //     return (m.affiliate !== '' || m.offer !== '' || m.cost !== 0.00); // v2 will have the option to do either all 3 filter or just to 2 so we see the entires with 0 cost
    ...
    function buildTableBodyEntries(merged) {
        ...
        const summary = merged.reduce(function (val, acc) {
            ...
            cpc = ( !isNaN(cost/clicks)  && isFinite(cost/clicks) ) ? (cost/clicks).toFixed(2) : '';
        ...
        function addCells(selection) {
            ...
            selection.selectAll('td')
                ...
                .append('td')
                .html(function (d) {
                    if (d.column === 'RPA' || d.column === 'Profit' || d.column === 'Revenue' || d.column === 'Cost' || d.column === 'CPC' || d.column === 'RPC') {
