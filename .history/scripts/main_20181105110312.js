    ...
    function buildTableBodyEntries(merged) {
        ...
        d3.select("#buttons").select("#first").on("click", function (d) {
            ...
            if (d.portion - tbodiesPerPage >= 0) {
                ...
                d3.select(this.parentNode).select('[data-id="previous-button"]').classed('disabled', true);
                d3.select(this.parentNode).select('[data-id="next-button"]').classed('disabled', false);
                remainingTbodies = totalBodies - d.portion;
                console.log('remaining ', remainingTbodies);
        ...
        d3.select("#buttons").select("#all-button").on("click", function (d) {
            // let the bodies hit the floor
            const remaining = totalBodies - d.portion;
            const showing = $('tbody:visible').length;
            if (showing < remaining) {
                d.portion += remaining;
                // redraw(d.portion);
            }
            console.log('total bodies ', totalBodies, ' portion is ', d.portion, ' and remain ', remaining);
        });
