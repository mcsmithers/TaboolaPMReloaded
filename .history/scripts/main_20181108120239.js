...
$(document).ready(function () {
    ...
    function performCleanupAndCalcs(merged) {
        ...
        merged = merged.map(element => {
            ...
            const clicks = Number.parseInt(element.clicks);
            const clicks = Number.parseInt(element.clicks).filter(m => {
                return (m.clicks !== 'NaN'); // v2 will have the option to do either all 3 filter or just to 2 so we see the entires with 0 cost
    
            });
        
            const actions = Number.parseInt(element.actions);
        
            const cost = Number.parseFloat(element.cost);
