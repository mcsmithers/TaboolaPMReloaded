...
$(document).ready(function () {
    ...
    function performCleanupAndCalcs(merged) {
        ...
        merged.forEach(function (element, index) {
            ...
            const actions = Number.parseFloat(merged[index].actions);
            if (!isNaN(actions)) {
                merged[index].actions = Number.parseFloat(actions).toFixed(0);
                calcs.push(actions);
            } else {
            ...
            if (!isNaN(revenue) && revenue !== 0) {
                merged[index].revenue = Number.parseFloat(revenue.toFixed(2));
                calcs.push(revenue);
            } else {
            ...
            const profitMargin = Number.parseFloat(profit / revenue).toFixed(0);
            if (Number.isFinite(profitMargin) || !isNaN(profitMargin)) {
                merged[index].profitMargin = Number.parseFloat(profitMargin);
                // console.log(profitMargin);
                calcs.push(profitMargin);
            }           
            else {
                // merged[index].profitMargin = '';
                calcs.push(profitMargin);
            }
    ...
    function buildTableBodyEntries(merged) {
        ...
        const summary = merged.reduce(function (val, acc) {
            ...
            val[acc.affiliateId].cost += Number.isFinite(acc.cost) ? Number.parseFloat(acc.cost) : 0;
            ...
            val[acc.affiliateId].name = acc.name;
