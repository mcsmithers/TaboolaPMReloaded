...
$(document).ready(function () {
    ...
    function performCleanupAndCalcs(merged) {
        ...
        merged = merged.map(element => {
            ...
            const clicks = !isNaN(element.clicks) ? (element.clicks) : '';

            const actions = !isNaN(element.actions) ? (element.actions) : '';

            const cost = !isNaN(element.cost) ? (element.cost) : '';

            const revenue = !isNaN(element.revenue) ? (element.revenue) : '';

            const profit = !isNaN(revenue - cost) ? (revenue - cost) : '';

            const profitMargin = !isNaN(profit / revenue) ? (profit / revenue) : '';

            const cpc = !isNaN(cost / clicks) || !isFinite(cost / clicks) ? (cost / clicks).toFixed(2) : '';
            ...
            console.log(typeof revenue, ' is revenue ', typeof cost, ' is cost ', typeof clicks, ' is clicks', typeof actions, ' is actions',);
