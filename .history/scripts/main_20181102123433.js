...
$(document).ready(function () {
    ...
    function buildTableBodyEntries(merged) {
        ...
        $('#table table')
            ...
            //     childrenSelector: 'table > tbody', // Paginate the rows with the `detail-row` class
            //     navigationWrapper: $('div#table-wrapper'), // Append the navigation menu to the `#myNavWrapper` div
    ...
    Promise.all([
        ...
        .then(data => {
            ...
            $("body").find("a").unbind("click");
            $('#table').prepend('<img id="error" src="error.gif" />')
            $('#errorContainer').html('Oh snap, something broke.  Try refreshing the page or try again later.');
            $('#errorContainer').css('display', 'block');
            $('#error-box').css('display', 'block');
            document.getElementById("errorContainer").style.color = "red";
            console.log(err);
