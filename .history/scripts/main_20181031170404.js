    ...
    function buildTableBodyEntries(merged) {
        ...
        $('.paginated').each(function () {
            ...
            $table.bind('repaginate', function () {
                ...
                var count = $('#table tr.detail-row').length;
                var shownRows = $table.find("tr.detail-row:not(:has(th))").length;
                $('#table').prepend('<div id="count">' + count + ' of ' + shownRows + ' total results</div>');
                if (startDateSelect != endDateSelect) {
                    $("tr.detail-row").css("display", "none"); // hides the row
                }
            });
