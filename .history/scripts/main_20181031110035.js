...
$(document).ready(function () {
    ...
    function failFunction(response) {
        ...
        $('#errorContainer').html('Oh no! Something broke. Try refreshing the page or come back later.');
    ...
    function buildTableBodyEntries(merged) {
        ...
        $('#table').on('pagerInitialized pagerComplete', function(event, data) {
            var pc = $('#table')[0].config.pager;
            console.log(pc);
                var perc = Math.round((pc.endRow / pc.filteredRows) * 100);
                $('#pager .pager_progress').width(perc + '%');
            console.log('--> ' + event.type, 'endRow: ' + pc.endRow, 'totleRows: ' + pc.totalRows, 'Percent: ' + perc, 'Page: ' + pc.page);
