...
$(document).ready(function () {
    ...
    function performCleanupAndCalcs(merged) {
        ...
        merged.forEach(function (element, index) {
            ...
            const clicks = Number.parseInt(merged[index].clicks);
            if (!isNaN(clicks)) {
            ...
            if (!isNaN(actions)) {
                merged[index].actions = Number.parseInt(actions);
                calcs.push(actions);
            } else {
            ...
            if (!isNaN(cost) && cost !== 0) {
                merged[index].cost = Number.parseFloat(cost);
                calcs.push(cost);
            } else {
                merged[index].cost = '';
                calcs.push(cost);
            ...
            if (!isNaN(revenue) && revenue !== 0) {
                merged[index].revenue = Number.parseFloat(revenue);
                calcs.push(revenue);
            } else {
            ...
            const campaign = merged[index].campaign;
            ...
            const profit = Number(cost - revenue);
            ...
            if (Number.isFinite(profitMargin) || !isNaN(profitMargin)) {
                merged[index].profitMargin = Number.parseFloat((profitMargin) * 100);
    ...
    function buildTableBodyEntries(merged) {
        ...
        const columnHeaderMap = {
            Date: "date",
            AffiliateIdOfferId: "affId_offId",
            Costd: "cost",
            Revenue: "revenue",
            CPC: "cpc",
            RPC: "rpc",
        ...
        const summary = merged.reduce(function (val, acc) {
            ...
            val[acc.affiliateId].actions += Number.isFinite(acc.actions) ? Number.parseFloat(acc.actions) : 0;

            val[acc.affiliateId].clicks += Number.isFinite(acc.clicks) ? Number.parseFloat(acc.clicks) : 0;

            val[acc.affiliateId].offer += Number.isFinite(acc.offer) ? Number.parseFloat(acc.offer) : 0;
            ...
            val[acc.affiliateId].campaign = acc.campaign;

            val[acc.affiliateId].affiliate = acc.affiliate;

            val[acc.affiliateId].offer = acc.offer;

            val[acc.affiliateId].cpc = Number.isFinite(val[acc.affiliateId].cost / val[acc.affiliateId].clicks) ? Number.parseFloat(acc.cpc) : 0;
            ...
            val[acc.affiliateId].rpa = Number.isFinite(Number.parseFloat(acc.revenue) / Number.parseFloat(acc.actions)) ? Number.parseFloat(acc.rpa) : 0;
    ...
    $("#end-date").change(function () {
        ...
        $('#report-table tbody').off('click', '.group');
        // $('#report-table').off('click', '.hideSummary');
