...
$(document).ready(function () {
    ...
    function performCleanupAndCalcs(merged) {
        ...
        merged.forEach(function (element, index) {
            ...
            if (!isNaN(spend) && spend !== 0) {
                merged[index].spend = Number(spend.toFixed(2).toLocaleString(undefined, {
                    minimumFractionDigits: 2,
                    maximumFractionDigits: 2
                }));
                calcs.push(spend);
        ...
        merged = merged.filter(m => {
            return (m.affiliate !== '' && m.offer !== '' && m.spend !== ''); // v2 will have the option to do either all 3 filter or just to 2 so we see the entires with 0 cost
    ...
    function buildTableBodyEntries(merged) {
        ...
        const summary = merged.reduce(function (val, acc) {
            ...
            val[acc.affId_offId].campaign = acc.campaign;
