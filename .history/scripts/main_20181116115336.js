...
$(document).ready(function () {
    ...
    function failFunction(response) {
        console.log("ERROR: ");
        console.log(response);
        $('#errorContainer').html('Oh no! Something is not available it seems.  Try refreshing the page or come back later.');
        ...
        $('#table table')
            ...
            .tablesorter({
                ...
                widgetOptions: {
                    ...
                    // Avoid dupes on redrawing
        if ($('.tablesorter-headerRow').length > 0 || $('.tablesorter-tfoot').length > 0) {
            console.log('thead ' + $('.tablesorter-headerRow').length);
            console.log('tfoot ' +  $('tfoot').length);
    ...
    Promise.all([
        ...
        .then(data => {
            ...
            $('#errorContainer').html('Oh snap, a data source is not responding.  Try refreshing the page or try again later.');
            $('#errorContainer').css('display', 'block');
            $('#error-box').css('display', 'block');
            document.getElementById("errorContainer").style.color = "red";
            console.log(err);
    ...
    $("#end-date").change(function () {
        ...
        // // UI cues and clearing from old loaders
        $("body").find("*").attr("disabled", "disabled");
        $("body").find("a").click(function (e) {
            e.preventDefault();
        });
